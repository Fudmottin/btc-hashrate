...
double sample_standard_deviation(const std::vector<int32_t>& values) {
   ...
   if (n < 2) return 0.0; // Standard deviation is undefined for n < 2
...
int main(int argc, char** argv) {
   ...
   if (argc > 2) {
      std::cerr << "Usage: " << argv[0] << " [days]\n"
                << "Hint: Run with no arguments for 1 day. Optionally pass "
                   "number of days as a floating-point value.\n";
      return 1;
   }
   ...
   std::cout << "Days: " << days << "\n"
             ...
             << "Blocks: " << block_delta << "\n"
             << "Next Diff Adjustment In: " << next_adjustment(current_height)
             << " Blocks\n"
             << "Expected Time: "
             << format_duration(static_cast<int>(days * 86400)) << "\n"
             << "Actual Time:   " << format_duration(time_delta) << "\n"
             << std::fixed << std::setprecision(2)
             << "Averge Block Time: " << avg_block_time / 60.0 << "m\n"
             << "Median Block Time: " << median_time(header_intervals) / 60.0
             << "m\n"
             << "Std Dev: "
             << sample_standard_deviation(header_intervals) / 60.0 << "m\n"
